...
export function band(index) {
  ...
  if (index < 0.58) return { label: "Indeterminate", tone: "#C08A16", short: "indeterminate" };
  if (index < 0.75) return { label: "Leans machine-assisted", tone: "#8A4BC4", short: "leans machine" };
...
export default function Verdict({ report, source }) {
  ...
  return (
    <section data-anim className="border border-[#CFC6B2] bg-[#F7F3EA]" style={{ animation: "riseIn 520ms ease-out both" }}>
      <div className="flex items-baseline justify-between border-b border-[#CFC6B2] px-5 py-2.5">
        <p className="font-['Fragment_Mono'] text-[12px] uppercase tracking-[0.18em] text-[#615A4A]">Synthetic-style index</p>
        <p className="font-['Fragment_Mono'] text-[12px] uppercase tracking-[0.14em]" style={{ color: b.tone }}>{relLabel}</p>
      </div>

      <div className="flex items-end gap-5 px-5 pt-5">
        <p className="font-['Hedvig_Letters_Serif'] text-[86px] leading-[0.78] tracking-[-0.02em]" style={{ color: b.tone }}>
          {report.index.toFixed(2).slice(1)}
        </p>
        <div className="pb-1.5">
          ...
          <p className="mt-1 font-['Fragment_Mono'] text-[12.5px] text-[#615A4A]">
            95% interval {lo.toFixed(2)} – {hi.toFixed(2)}
          </p>
        </div>
      </div>
      ...
      <div className="px-5 pt-5">
        ...
        <div className="flex justify-between font-['Fragment_Mono'] text-[12px] uppercase tracking-[0.12em] text-[#615A4A]">
          <span>.00 human habits</span>
          <span>.50</span>
          <span>machine habits 1.0</span>
        </div>
        <p className="mt-2.5 text-[14px] leading-[1.5] text-[#615A4A]">{relNote}</p>
      </div>

      <dl className="mt-5 grid grid-cols-3 border-t border-[#CFC6B2] text-center">
        {[
          ...
          ["reading ease", s.flesch.toFixed(0)],
        ].map(([k, v], i) => (
...
function buildReport(r, source) {
  ...
  L.push("");
  L.push(`synthetic-style index: ${r.index.toFixed(3)}  (95% CI ${Math.max(0, r.index - r.half).toFixed(2)}–${Math.min(1, r.index + r.half).toFixed(2)})`);
  ...
  L.push(`mean sentence ${r.stats.meanLen.toFixed(1)}w · surprisal ${r.stats.surpMean.toFixed(2)}±${r.stats.surpSd.toFixed(2)} bits · Zipf slope ${r.stats.zipfSlope.toFixed(3)} · Flesch ${r.stats.flesch.toFixed(1)}`);
  L.push("");
  ...
  L.push("");
  L.push("CAVEAT: this instrument measures style, not origin. It is not Turnitin and does not");
  L.push("reproduce Turnitin's proprietary classifier. Non-native English, edited text, legal");
  L.push("boilerplate and short samples all read as more synthetic than they are. Do not take");
  L.push("a decision affecting a person on this output alone.");
  return L.join("\n");
}
