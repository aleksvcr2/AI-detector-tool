import {
  surprisal, inVocab, FUNCTION_WORDS, DISCOURSE_MARKERS, REGISTER_MARKERS,
  HEDGES, INTENSIFIERS, FIRST_PERSON, ABBREVIATIONS,
} from "./lexicon.js";
...
export function sentences(text) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (c === ".") {
      ...
      const tok = (before.match(/[A-Za-z.'’]+$/) || [""])[0].replace(/\./g, "").toLowerCase();
  ...
  return out.filter((s) => /[A-Za-z]/.test(s.text));
}
...
export function syllables(w) {
  ...
  let t = s.replace(/(?:[^laeiouy]es|ed|[^laeiouy]e)$/, "").replace(/^y/, "");
...
export async function analyze(raw, onProgress = () => {}) {
  const text = raw.replace(/\r\n?/g, "\n");
  onProgress({ phase: "Segmenting", pct: 0.02 });
  await yieldToUI();
  ...
  const DET = new Set("the a an this that these those his her its their our my your".split(" "));
  const PRON = new Set("i we you he she they it one someone everyone".split(" "));
  const CONJ = new Set("and but or so yet nor because although though while if when since as".split(" "));
  const ADV = new Set(DISCOURSE_MARKERS.map((m) => m.split(" ")[0]));
  const PASSIVE_RE = /\b(is|are|was|were|be|been|being|am)\s+(?:\w+ly\s+)?(\w{3,}(?:ed|en)|done|made|given|taken|shown|seen|held|known|found|used|built|sent|kept|left|brought|put|set)\b/i;
  ...
  for (let i = 0; i < sents.length; i++) {
    ...
    if (w.length) {
      ...
      for (let k = 0; k < w.length; k++) {
        ...
        surpSum += surprisal(lw);
        if (FUNCTION_WORDS.has(lw)) funcCount++;
        if (!inVocab(lw)) oov++;
        if (FIRST_PERSON.has(lw)) firstPersonN++;
        if (INTENSIFIERS.includes(lw)) intensN++;
        const L = Math.min(16, w[k].length);
        lenHist.set(L, (lenHist.get(L) || 0) + 1);
      ...
      else if (ADV.has(o) || o.endsWith("ly")) openerClass[3]++;
      else if (/^[A-Z]/.test(w[0])) openerClass[4]++;
      else openerClass[5]++;
      ...
      let dm = 0;
      for (const m of DISCOURSE_MARKERS) if (lower.includes(m)) dm++;
      discourseN += dm;
      let rm = 0;
      for (const m of REGISTER_MARKERS) if (lower.includes(m)) rm++;
      regMarkN += rm;
      let hg = 0;
      for (const m of HEDGES) if (new RegExp(`\\b${m}\\b`).test(lower)) hg++;
      hedgeN += hg;

      sentRows.push({
        i, offset: s.offset, nl: s.nl || 0, text: s.text, n: w.length, surp: surpMean,
        dm, rm, passive,
        human: (contractionN, /\b(i|we|my|our|me|us)\b/i.test(s.text) ? 1 : 0),
      });
    }
    if (i % CHUNK === CHUNK - 1) {
  ...
  const paraSentCounts = paras.map((p) => Math.max(1, sentences(p).length));
  const grades = [];
  ...
  const fk = totalWords ? 206.835 - 1.015 * (totalWords / Math.max(1, sentRows.length)) - 84.6 * (syllTotal / totalWords) : 0;
  ...
  const slips =
    (text.match(/[a-z],[A-Za-z]/g) || []).length +
    (text.match(/[a-z]\.[A-Z]/g) || []).length +
    (text.match(/\b([a-z]{3,})\s+\1\b/gi) || []).length +
    (text.match(/\bi\b/g) || []).length * 0.35 +
    (text.match(/[!?]{2,}|\.{4,}/g) || []).length +
    (text.match(/\s+[,.;:]/g) || []).length +
    (text.match(/\b[A-Z]{2,}[a-z]+/g) || []).length * 0.5;
  ...
  const foundRegister = [];
  {
    ...
    for (const m of REGISTER_MARKERS) {
      let c = 0, idx = 0;
  ...
  return {
    index, half, logit, signals, families, rows,
    stats: {
