...
export default function App() {
  ...
  const run = useCallback(async (raw, label, extraNote) => {
    setError(null);
    ...
    try {
      const r = await analyze(raw, setProgress);
      setReport(r);
    } catch (e) {
    ...
    <div className="min-h-screen bg-[#EDE7DA] text-[#211F1A] antialiased selection:bg-[#5B33C7] selection:text-[#F7F3EA]">
      ...
      <div className="min-h-screen" style={PAPER}>
        ...
        <main className="mx-auto w-full max-w-[1360px] px-5 pb-24 sm:px-8">
          <Intake onFile={onFile} onText={run} busy={busy} progress={progress} error={error} note={note} source={source} hasReport={!!report} />
    ...
    <header className="sticky top-0 z-30 border-b border-[#000]/40 bg-[#191B18] text-[#EDE7DA] shadow-[0_1px_0_rgba(255,255,255,.06)_inset]">
      <div className="mx-auto flex w-full max-w-[1360px] flex-wrap items-baseline gap-x-5 gap-y-1 px-5 py-3 sm:px-8">
        ...
        <p className="ml-auto font-['Fragment_Mono'] text-[12px] uppercase tracking-[0.14em] text-[#EDE7DA]/90">
          {count} parameters · nothing leaves this tab
        </p>
      </div>
    </header>
  );
      ...
      <div className="relative overflow-hidden border border-[#C0341C]/35 bg-[#F7F3EA]">
        ...
        <div className="grid gap-5 p-6 pl-8 sm:grid-cols-[190px_minmax(0,1fr)] sm:p-7 sm:pl-9">
          ...
          <div className="space-y-3 text-[15px] leading-[1.62] text-[#3B372F]">
            ...
            <p>
              It is not Turnitin. Turnitin's detector is a proprietary neural classifier trained on labelled student
              submissions, and its published metrics are not public; nothing running locally in a browser can reproduce its
              numbers. What you get here is the open, inspectable layer underneath commercial detectors: burstiness,
              surprisal variance, lexical diversity, register, structure, typography. Every value is shown with the reference
              band it is being judged against, so you can disagree with the instrument.
            </p>
            <p className="text-[#615A4A]">
              Known failure modes: non-native English writers, heavily edited or copy-edited text, technical and legal
              boilerplate, translations, and any human who writes in a formal, even register all read as more synthetic than
              they are. Machine text that a person has rewritten reads as more human than it is. Short samples are close to
              worthless: under 350 words the confidence interval swallows the answer. Never take a decision that affects a
              person on this output alone.
            </p>
          </div>
        </div>
      </div>
